...
///require "mail.php"; 
    if(!$con =  mysqli_connect("localhost", "root","","seaport")){
    ...
    function send_email($email){
    	...
    	$query = "insert into passreset (email,code, expire) value ('$email','$code','$expire')";
    ...
    function save_password($password){
     ...
     $password = password_hash($password, PASSWORD_DEFAULT);
    	...
    	$query = "update adminpass set password = '$password' where email ='$email' limit 1";
    	mysqli_query($con,$query);
    }
     ...
     function valid_email($email){
      ...
      global $con;
    	...
    	$query = "select * from adminpass where email ='$email' limit 1";
    ...
    function is_code_correct($code){
     ...
     $query = "select * from passreset where code = '$code' && email = '$email' && expire > '$expire' order by id desc limit 1";
     	$result = mysqli_query($con,$query);
     	if($result) {
     		if(mysqli_num_rows($result) >0)
     		{
     ...
     switch ($mode) {
       ...
       <form method = "post" action = "forgotpass.php?mode=enter_email"> 
					...
					<input type="submit" value="Next">
				 	<br><br>
				 	<div><a href = "administratorlogin.php">Login </a></div>
				 </form>
				 <?php
    			break;
